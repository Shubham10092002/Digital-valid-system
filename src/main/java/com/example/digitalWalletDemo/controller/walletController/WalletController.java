package com.example.digitalWalletDemo.controller.walletController;
...
//@Transactional(readOnly = true)
    ...
    public ResponseEntity<?> getAllWallets() {
        try {
            ...
            //logger.error("Error fetching wallets", e);
...
//    }
    ...
    public ResponseEntity<?> createWalletForUser(
        ...
        try {
            WalletResponseDTO walletResponse = walletService.createWalletForUser(userId, walletDTO);
            return ResponseEntity.status(HttpStatus.CREATED).body(walletResponse);
        } catch (UserNotFoundException e) {
            //logger.error("User not found: {}", userId);
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of("errorCode", "USER_NOT_FOUND", "reason", e.getMessage()));
        } catch (Exception e) {
            //logger.error("Error creating wallet for user {}: {}", userId, e.getMessage(), e);
            return ResponseEntity.internalServerError()
                    .body(Map.of("errorCode", "UNKNOWN_ERROR", "reason", e.getMessage()));
        }
    }
